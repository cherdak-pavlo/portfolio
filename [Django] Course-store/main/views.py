...
def add_to_cart(request, product_id):
    if Product.objects.get(id=product_id):
        ...
        aggregated_sum = Product.objects.filter(id__in=request.session.get("products", [])).aggregate(Sum("price"))
        total_sum = aggregated_sum.get("price__sum", 0)
        request.session["cart_total_price"] = total_sum
